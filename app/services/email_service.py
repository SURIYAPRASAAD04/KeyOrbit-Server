...
class EmailService:
    ...
    def _send_email(to_email, subject, html_content):
        """Internal method to send email via Gmail SMTP with SSL"""
        try:
            ...
            # Connect to Gmail SMTP server with SSL (port 465)
            logger.info(f"Connecting to {Config.SMTP_SERVER}:{Config.SMTP_PORT} with SSL")
            
            server = smtplib.SMTP_SSL(Config.SMTP_SERVER, Config.SMTP_PORT, timeout=30, context=context)
    ...
    def send_welcome_email(email, name):
        ...
        <div style="text-align: center; margin: 30px 0;">
            <a href="{Config.FRONTEND_URL}/dashboard" style="display: inline-block; padding: 16px 32px; background: rgba(242, 140, 0, 0.2); backdrop-filter: blur(20px); color: white; text-decoration: none; border-radius: 16px; font-weight: 600; font-size: 16px; margin: 25px 0; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid rgba(242, 140, 0, 0.3); box-shadow: 0 8px 32px rgba(242, 140, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.2); position: relative; overflow: hidden; letter-spacing: 0.5px;">
    ...
    def send_user_invitation_email(email, invitation_token, organization_name, inviter_name, role, message="", is_resend=False):
        ...
        {f'<p style="color: rgba(255, 255, 255, 0.8); font-size: 15px; line-height: 1.6; margin: 15px 0; font-style: italic;">"{message}"</p>' if message else ''}
    ...
    def send_admin_notification_email(admin_email, user_email, user_name):
        ...
        <div style="text-align: center; margin: 30px 0;">
            <a href="{Config.FRONTEND_URL}/admin/users" style="display: inline-block; padding: 16px 32px; background: rgba(242, 140, 0, 0.2); backdrop-filter: blur(20px); color: white; text-decoration: none; border-radius: 16px; font-weight: 600; font-size: 16px; margin: 25px 0; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid rgba(242, 140, 0, 0.3); box-shadow: 0 8px 32px rgba(242, 140, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.2); position: relative; overflow: hidden; letter-spacing: 0.5px;">
